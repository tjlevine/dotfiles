...
# enable __git_ps1 if git-prompt.sh is installed to ~/bin to enable custom git-aware PS1
if [ -f ~/bin/git-prompt.sh ]; then
    source ~/bin/git-prompt.sh

    # custom PS1 to show git branches in git dirs
    PS1='\[$(tput setaf 4)\]\u\[$(tput setaf 2)\]@\[$(tput setaf 5)\]\h:\[$(tput setaf 3)\]\w\[$(tput bold)\]$(__git_ps1 " (%s)")\[$(tput sgr0)\] \$ '
else
    # custom PS1 without __git_ps1
